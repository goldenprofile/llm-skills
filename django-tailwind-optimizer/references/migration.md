...
### Установка standalone CLI v3

Фиксируй тег v3-линейки (последний — v3.4.x), НЕ `latest` (он отдаст v4). Суффикс
платформы — свой (`linux-x64`, `macos-arm64`, `macos-x64`, `windows-x64.exe`):
...
```bash
./tailwindcss -i static/src/tailwind.css -o static/dist/tailwind.min.css --minify
```

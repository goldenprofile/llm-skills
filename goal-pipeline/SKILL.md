...
name: goal-pipeline
description: >
  Планировщик-исполнитель поверх нативной /goal Claude Code: recon проекта,
  декомпозиция на фазы с измеримыми критериями, вшитые гейты
...
1. **`/goal` доступен.** Проверка: команда принимается из ввода и появляется
...
2. **Auto mode для непрерывного прогона.** `/goal` снимает подтверждение
...
3. **Git-репозиторий** — страховка. Прогон правит исходники автономно; без git
   откат болезненный. Если репо нет — предложи `git init` и первый коммит до старта.
4. **Понятные команды** проекта (build/type/lint/test) — `make`-цели или
...
| **full-auto** | После ревью плана руки прочь до `GP_RUN_COMPLETE`; останавливается на 3-strike блоке или когда исчерпан `Fix budget`. | Низкорисковая задача (чистый рефакторинг под git, добавление тестов). |
...
2. **Git baseline** — `git rev-parse HEAD`. Запиши в `STATE.md` как
...
- **Mandatory commands** (что обязано пройти зелёным: `make`-цели или прямые команды);
...
2. Проверь, что все `.goalrun/phases/phase-N.md` существуют и содержат маркер
   `GP_PHASE_START`.
3. Напечатай fenced-блок с **готовой строкой `/goal`**, подставив `Turn cap`

...
1. **`/goal` включён** в этой сборке Claude Code. Проверка: команда `/goal`
   принимается из ввода и появляется индикатор `◎ /goal active`. Если команды нет
   (другая сборка или другой ИИ-ассистент) — скажи об этом прямо и предложи
   `ralph-loop`: та же дисциплина фаз, но петля внутри текущей сессии, без
   хостового оценщика. Не изображай прогон, которого среда не поддерживает.
...
3. **Понятные команды** проекта (build/type/lint/test) — `make`-цели или

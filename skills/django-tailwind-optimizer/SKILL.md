...
description: >
  Tailwind CSS в Django-шаблонах: перевод с CDN на production-сборку (v4,
  CSS-first), поиск дублирующихся наборов классов, вынос повторов в
  компоненты, подготовка финального CSS к деплою. Используй когда
  пользователь говорит «Tailwind пора перевести с CDN на нормальную сборку»,
  «оптимизируй tailwind», «убери дублирование стилей», «подготовь CSS к
  продакшену», или правит Django-шаблоны с классами Tailwind. Общий
  визуальный дизайн и типографика — frontend-design.
metadata:
  version: 2.1.1
---
...
Возьми бинарник под свою платформу: `tailwindcss-linux-x64`, `tailwindcss-macos-arm64`,
`tailwindcss-macos-x64` или `tailwindcss-windows-x64.exe` того же тега.

```bash
# Linux / macOS (подставь свой суффикс платформы)
v=v4.3.2
curl -sLo tailwindcss "https://github.com/tailwindlabs/tailwindcss/releases/download/$v/tailwindcss-linux-x64"
chmod +x tailwindcss
```
...
```bash
# Linux / macOS
./tailwindcss -i static/src/tailwind.css -o static/dist/tailwind.css --watch          # dev
./tailwindcss -i static/src/tailwind.css -o static/dist/tailwind.min.css --minify     # prod
```

...
description: >
  Миграция БД перед прод-деплоем: заблокирует ли таблицу, уронит ли прод,
  переживёт ли старый код новую схему. Django migrations и Alembic, Postgres
  и SQLite — блокировки, downtime, потеря данных, опасный backfill,
  необратимые операции, expand/contract. Используй когда пользователь
  спрашивает «безопасна ли миграция», «не уронит ли прод», «мне с сервера
  сделать commit или merge миграции», добавляет колонку, индекс или
  constraint на большой таблице, пишет data-миграцию, или упоминает
  CONCURRENTLY, RunPython, zero-downtime. Подобрать сам индекс под запрос —

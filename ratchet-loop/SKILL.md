...
name: ratchet-loop
description: >
  In-session оптимизационная петля-храповик: берёт ОДИН измеримый скаляр и тянет
  его до упора, оставляя только изменения, которые его улучшили, а остальные
  откатывая через git. Ядро: один мутируемый артефакт, замороженный оценщик
  (anti-Goodhart), одна метрика, фиксированный бюджет на попытку, keep-or-revert
  как храповик, лог попыток на диск, независимый verifier-проход в Definition of
  Done. В отличие от gate-петель
  («чини пока зелёное») оптимизирует НЕПРЕРЫВНЫЙ скаляр и сам не терминируется —
  крутится до бюджета/плато/прерывания. Используй когда пользователь говорит
  «оптимизируй <метрику> до упора», «храповик / ratchet», «крути пока улучшается»,
  «minimize/maximize <X> до плато», «жми <метрику> и откатывай регрессии», «прогон
  на снижение latency/запросов/размера».
  НЕ goal-pipeline (тот — coverage под хостовый /goal, терминируется на «бэклог
  пуст», без скаляра и отката). НЕ ralph-loop (та — общая in-session петля без
  контракта оптимизации; ratchet-loop её специализация со скаляром + frozen eval +
  keep-or-revert + verifier). НЕ gate-loop / `/goal "done when зелёное"` (булев
  гейт, терминируется). НЕ spec-writer (тот только пишет документ).
metadata:
  version: 1.0.0
---
...
`contract-example.md` (снижение числа SQL-запросов на странице Django).
